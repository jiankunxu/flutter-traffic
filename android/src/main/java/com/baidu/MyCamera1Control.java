...
public class MyCamera1Control implements ICameraControl {
    ...
    public MyCamera1Control(Context context) {
    ...
    Camera.PreviewCallback previewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(final byte[] data, Camera camera) {
            ...
            CameraThreadPool.execute(new Runnable() {
                @Override
                public void run() {
                    MyCamera1Control.this.onRequestDetect(data);
                }
            });
        }
    };
    ...
    private void initCamera() {
        try {
            if (camera == null) {
                try {
                    camera = Camera.open(new Integer(MyCameraActivity.TYPE));
                } catch (Throwable e) {
                    e.printStackTrace();
                    startPreview(true);
                    return;
    ...
    private void startAutoFocus() {
        CameraThreadPool.createAutoFocusTimerTask(new Runnable() {
            @Override
            public void run() {
                synchronized (MyCamera1Control.this) {
