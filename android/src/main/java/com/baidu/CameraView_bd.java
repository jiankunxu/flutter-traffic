 ...
 */
public class CameraView_bd extends FrameLayout {
    ...
    public CameraView_bd(Context context) {
        super(context);
        init();
    }

    public CameraView_bd(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public CameraView_bd(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }
    private void init() {
        cameraControl = new MyCamera1Control(getContext());

        displayView = cameraControl.getDisplayView();
        addView(displayView);

        maskView = new MyMaskView(getContext());
        addView(maskView);
    ...
    public void setMaskType(@MyMaskView.MaskType int maskType, final Context ctx) {
        maskView.setMaskType(maskType);

        maskView.setVisibility(VISIBLE);
        if (MyCameraActivity.ISSHOWTYPE.equals("1")) {
            hintView.setVisibility(VISIBLE);
        } else {
            hintView.setVisibility(GONE);
        }

        int hintResourceId = R.drawable.camera_flip_bg_gray;
        this.maskType = maskType;
        hintView.setImageResource(hintResourceId);
        hintViewTextWrapper.setVisibility(INVISIBLE);
//        }

        if (maskType == MyMaskView.MASK_TYPE_ID_CARD_FRONT && isEnableScan) {
        ...
        if (maskType == MyMaskView.MASK_TYPE_ID_CARD_BACK && isEnableScan) {
            cameraControl.setDetectCallback(new ICameraControl.OnDetectPictureCallback() {
                @Override
                public int onDetect(byte[] data, int rotation) {
                    return detect(data, rotation);
    ...
    private int detect(byte[] data, final int rotation) {
        ...
        // 调用本地质量控制请求
        switch (maskType) {
            case MyMaskView.MASK_TYPE_ID_CARD_FRONT:
                status = IDcardQualityProcess.getInstance().idcardQualityDetectionImg(bitmap, true);
                break;
            case MyMaskView.MASK_TYPE_ID_CARD_BACK:
    ...
    private Bitmap crop(File outputFile, byte[] data, int rotation) {
        try {
            ...
            if (MyCameraActivity.TYPE.equals(MyCameraActivity.TYPE_FRONT)) {
